...
import 'package:eznext/api_models/student_documents.dart';
...
class _MyDocumentsState extends State<MyDocuments> {
  ...
  final myKey = new GlobalKey<_MyDocumentsState>();
...
// NOTE: even you didnt select item this method will be called with null of value so you should call your call back with checking if value is not null
  ...
  Future<void> _startDownload(String savePath, String url) async {
    ...
    if (schoolcode == '') {
      setState(() {
        _fileUrl =
            siteurl + 'uploads/student_documents' + '/' + stdid + '/' + url;
      });
    } else {
      setState(() {
        _fileUrl = 'https://' +
  ...
  Future getDocuments() async {
    try {
      var rsp = await Stud_Docs(stdid, token.toString(), uid);
  ...
  Future getdirectory(String url) async {
    ...
    if (isPermissionStatusGranted) {
      final Directory _appDocDirFolder =
          Directory('${dir!.path}/$Appname/mydocs');
      if (await _appDocDirFolder.exists()) {
        //debugPrint('exists');
        appDocDir = await _getDownloadDirectory();
        final String dirPath = '${appDocDir!.path}/$Appname/mydocs';
