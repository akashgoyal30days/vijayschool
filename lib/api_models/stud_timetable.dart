...
Future Stud_timetable(String stdid, String stoken, String uid) async {
  var uri = '';
  if(schoolcode == '') {
    SharedPreferences initialschoolcode = await SharedPreferences.getInstance();
    var url = initialschoolcode.getString('url').toString();
    uri  = '${url}webservice/class_schedule';
  }
  if(schoolcode != ''){
    uri = "https://$schoolcode.$baseurl/webservice/class_schedule";
  }
  //debugPrint('url- $uri');
  Map bodys = {
    'student_id': stdid
