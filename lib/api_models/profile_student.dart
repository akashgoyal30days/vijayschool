...
Future Stud_profile(String stdid, String stoken, String uid) async {
  var uri = '';
  if(schoolcode == '') {
    SharedPreferences initialschoolcode = await SharedPreferences.getInstance();
    var url = initialschoolcode.getString('url').toString();
    uri  = '${url}webservice/getstudentprofile';
  }
  if(schoolcode != ''){
    uri = "https://$schoolcode.$baseurl/webservice/getstudentprofile";
  }
  //debugPrint('url- $uri');
  Map bodys = {
    'student_id': stdid
