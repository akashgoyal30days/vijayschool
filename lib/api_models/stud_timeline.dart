...
Future Stud_timeline(String stdid, String stoken, String uid) async {
  print(stdid);
  var uri = '';
  if (schoolcode == '') {
    SharedPreferences initialschoolcode = await SharedPreferences.getInstance();
    var url = initialschoolcode.getString('url').toString();
    uri = '${url}webservice/gettimeline';
  }
  if (schoolcode != '') {
    uri = "https://$schoolcode.$baseurl/webservice/gettimeline";
  }
  //debugPrint('url- $uri');
  Map bodys = {'studentId': stdid};
